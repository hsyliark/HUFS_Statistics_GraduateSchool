...
dat.gen <- function(n, beta, seed=NULL){
  ...
  x10 <- runif(n,-10,10) ; x11 <- runif(n,-10,10)
  x <- cbind(1, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11)
  linpred <- x%*%beta
  p <- exp(x%*%beta) / (1+exp(x%*%beta))
  y <- apply(as.matrix(p), c(1,2), rbinom, n=1, size=1)
  dat <- data.frame(y=y, x1=x1, x2=x2, x3=x3, x4=x4, x5=x5,
                    x6=x6 , x7=x7, x8=x8, x9=x9, x10=x10, x11=x11, linpred=linpred)
  return( dat )
}
...
for (i in 1:50) {
  ...
  test.sim <- dat.gen(500, beta)
  
  train.sim <- train.sim[,-13] ; test.sim <- test.sim[,-13]
...
### 3. Kernel ridge logistic regression classification using Bagging (KRLCB)
...
for (s in 1:50) {
  ...
  y.hat.bag[mean.hat.bag == 0.5] <- NA
  
  KRLCB[s] <- mean(y.hat.bag != test.sim.y, na.rm=T)
...
dat1.3 <- data.frame(test.MR=KRLCB, method=rep("KRLCB",50), number=rep("50",50))
...
### 4. Kernel ridge logistic regression classification using Random Forest (KRLCR)
...
for (s in 1:50) {
  ...
  train.sim.X <- train.sim[,2:12] ; test.sim.X <- test.sim[,2:12]
  ...
  y.hat.rf[mean.hat.rf == 0.5] <- NA
  
  KRLCR[s] <- mean(y.hat.rf != test.sim.y, na.rm=T)
...
dat1.4 <- data.frame(test.MR=KRLCR, method=rep("KRLCR",50), number=rep("50",50))



dat1 <- rbind(dat1.1, dat1.2, dat1.3, dat1.4)
...
dat2.3 <- data.frame(test.MR=KRLCB, method=rep("KRLCB",50), number=rep("100",50))
...
dat2.4 <- data.frame(test.MR=KRLCR, method=rep("KRLCR",50), number=rep("100",50))



dat2 <- rbind(dat2.1, dat2.2, dat2.3, dat2.4)
...
dat3.1 <- data.frame(test.MR=KRLC, method=rep("KRLC",50), number=rep("150",50))
...
dat3.2 <- data.frame(test.MR=KRLCS, method=rep("KRLCS",50), number=rep("150",50))
...
dat3.3 <- data.frame(test.MR=KRLCB, method=rep("KRLCB",50), number=rep("150",50))
...
dat3.4 <- data.frame(test.MR=KRLCR, method=rep("KRLCR",50), number=rep("150",50))



dat3 <- rbind(dat3.1, dat3.2, dat3.3, dat3.4)
...
dat4.1 <- data.frame(test.MR=KRLC, method=rep("KRLC",50), number=rep("200",50))
...
dat4.2 <- data.frame(test.MR=KRLCS, method=rep("KRLCS",50), number=rep("200",50))
...
dat4.3 <- data.frame(test.MR=KRLCB, method=rep("KRLCB",50), number=rep("200",50))
...
dat4.4 <- data.frame(test.MR=KRLCR, method=rep("KRLCR",50), number=rep("200",50))



dat4 <- rbind(dat4.1, dat4.2, dat4.3, dat4.4)
...
ggplot(final.dat, aes(x = number, y = test.MR, fill = method))
