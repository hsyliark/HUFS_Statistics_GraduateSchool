...
my.kernel.logistic <- function(y, K, lambda) {
  ...
  for (i in 1:100) {
    ...
    diff1 <- abs(sum(y*new.logit.hat - log(1+exp(new.logit.hat))) - sum(y*old.logit.hat - log(1+exp(old.logit.hat))))
    diff2 <- abs(sum(y*old.logit.hat - log(1+exp(old.logit.hat))))
    diff <- diff1 / diff2
...
fit.ftn <- function(n.train, p, beta) {
  ...
  for (i in 1:100) {
    ...
    idx <- which.min(mean.logL)
    best.lam <- h$lambda[ mean.logL == mean.logL[idx] ]
  ...
  for (i in 1:100) {
    ...
    for (j in 1:50) {
      ...
      best.lam.c <- hc$lambda[ mean.logL.c == mean.logL.c[idx.c] ]
...
dat1.1 <- fit.ftn(50, 5, 0.5^(1:5)) # Choose when n.train=50
dat1.2 <- fit.ftn(100, 5, 0.5^(1:5)) # Choose when n.train=100
dat1.3 <- fit.ftn(200, 5, 0.5^(1:5)) # Choose when n.train=200
dat1.4 <- fit.ftn(400, 5, 0.5^(1:5)) # Choose when n.train=400
...
dat2.1 <- fit.ftn(50, 10, 0.5^(1:10)) # Choose when n.train=50
dat2.2 <- fit.ftn(100, 10, 0.5^(1:10)) # Choose when n.train=100
dat2.3 <- fit.ftn(200, 10, 0.5^(1:10)) # Choose when n.train=200
dat2.4 <- fit.ftn(400, 10, 0.5^(1:10)) # Choose when n.train=400
...
dat3.1 <- fit.ftn(50, 20, 0.5^(1:20)) # Choose when n.train=50
dat3.2 <- fit.ftn(100, 20, 0.5^(1:20)) # Choose when n.train=100
dat3.3 <- fit.ftn(200, 20, 0.5^(1:20)) # Choose when n.train=200
dat3.4 <- fit.ftn(400, 20, 0.5^(1:20)) # Choose when n.train=400
...
dat4.1 <- fit.ftn(50, 40, 0.5^(1:40)) # Choose when n.train=50
dat4.2 <- fit.ftn(100, 40, 0.5^(1:40)) # Choose when n.train=100
dat4.3 <- fit.ftn(200, 40, 0.5^(1:40)) # Choose when n.train=200
dat4.4 <- fit.ftn(400, 40, 0.5^(1:40)) # Choose when n.train=400
